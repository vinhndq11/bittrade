...
var create_img = function create_img(settings) {
  let $img = JQ('<img/>').attr('src', create_canvas(settings)[0].toDataURL('image/png')).addClass(settings.className || '');
  if(settings.data){
  	Object.keys(settings.data).forEach(key => $img.attr('data-' + key, settings.data[key]));
  }
  return $img;
